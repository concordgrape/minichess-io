...
exports.generateEndgamePuzzle = generateEndgamePuzzle;
...
function edgeRankFile() {
    const bias = () => (0, chessSearch_1.pick)([0, 0, 1, 2, 5, 6, 7, 7]);
    return (0, chessSearch_1.sq)(bias(), bias());
}
// ─── rook-endgame & zugzwang: random placement + reject ─────────────────────────
function buildPlacement(pieces) {
...
function hasQuietMatingKey(fen) {
    const keys = (0, chessSearch_1.matingFirstMoves)(fen, 2);
...
function buildKingAndPawn() {
    ...
    const pr = (0, chessSearch_1.pick)([5, 6]);
    const ps = (0, chessSearch_1.sq)(pf, pr);
    board[ps] = "P";
    // White king shepherds the pawn (must be close to support promotion + mate).
    let wk = -1;
    for (let t = 0; t < 60; t++) {
        const s = (0, chessSearch_1.sq)(Math.min(7, Math.max(0, pf + (0, chessSearch_1.randInt)(5) - 2)), Math.min(7, Math.max(0, pr + (0, chessSearch_1.randInt)(5) - 2)));
        if (s !== ps) {
            wk = s;
            break;
        }
    ...
    let bk = -1;
    for (let t = 0; t < 60; t++) {
        const s = (0, chessSearch_1.sq)(Math.min(7, Math.max(0, pf + (0, chessSearch_1.randInt)(7) - 3)), Math.min(7, Math.max(0, pr + (0, chessSearch_1.randInt)(5) - 1)));
        if (s !== ps && s !== wk && kingsOk(s, wk)) {
            bk = s;
            break;
        }
    }
...
function generateEndgamePuzzle(gameId, difficulty) {
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (gameId === "king-and-pawn") {
            ...
            const pieces = gameId === "rook-endgame" ? ["R"] : [(0, chessSearch_1.pick)(["Q", "R"])];
            fen = buildPlacement(pieces);
        }
        if (!fen || !(0, chessSearch_1.loadPlayable)(fen))
            continue;
        if (!(0, chessSearch_1.exactlyMateIn)(fen, 2))
            continue;
        if (gameId === "zugzwang" && !hasQuietMatingKey(fen))
            continue;
        return { fen, difficulty };
    }
    return { fen: FALLBACKS[gameId], difficulty };
}
//# sourceMappingURL=endgameGenerator.js.map