...
exports.publishGamesBatch = publishGamesBatch;
 ...
 */
async function publishGamesBatch(files, commitMessage) {
    var _a;
    if (files.length === 0)
        return;
    ...
    const branch = (_a = process.env.GITHUB_BRANCH) !== null && _a !== void 0 ? _a : "main";
    const headers = githubHeaders();
    // 1. Resolve HEAD
    const refRes = await fetch(`${GITHUB_API}/repos/${repo}/git/refs/heads/${branch}`, { headers });
    if (!refRes.ok)
        throw new Error(`GitHub GET ref failed (${refRes.status}): ${await refRes.text()}`);
    ...
    if (!commitRes.ok)
        throw new Error(`GitHub GET commit failed (${commitRes.status}): ${await commitRes.text()}`);
    ...
    const treeItems = await Promise.all(files.map(async ({ path, content }) => {
        ...
        if (!blobRes.ok)
            throw new Error(`GitHub blob failed for ${path} (${blobRes.status}): ${await blobRes.text()}`);
    ...
    if (!treeRes.ok)
        throw new Error(`GitHub tree failed (${treeRes.status}): ${await treeRes.text()}`);
    ...
    if (!newCommitRes.ok)
        throw new Error(`GitHub commit failed (${newCommitRes.status}): ${await newCommitRes.text()}`);
    const newCommitSha = (await newCommitRes.json()).sha;
    // 6. Advance branch ref
    const updateRes = await fetch(`${GITHUB_API}/repos/${repo}/git/refs/heads/${branch}`, { method: "PATCH", headers, body: JSON.stringify({ sha: newCommitSha }) });
    if (!updateRes.ok)
        throw new Error(`GitHub update ref failed (${updateRes.status}): ${await updateRes.text()}`);
